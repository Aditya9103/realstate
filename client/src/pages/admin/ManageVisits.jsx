    ...
    <div className="font-sans space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage property tour and visit requests.</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {isLoading ? (
          <div className="p-8 text-center text-gray-500">Loading visit requests...</div>
        ) : isError ? (
          <div className="p-8 text-center text-red-500">{error?.data?.message || 'Failed to load visits'}</div>
        ) : visits.length === 0 ? (
          <div className="p-8 text-center text-gray-500">No visit requests found.</div>
        ) : (
                  ...
                  <tr key={visit._id} className={`hover:bg-gray-50 transition-colors ${visit.status === 'Pending' ? 'bg-[#fcf9f2]/30' : ''}`}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 font-bold text-[#1a2b3c] mb-1">
                        <User size={14} className="text-gray-400" /> {visit.name}
                      </div>
